...
export repo_name="tenx_job_recommender" #used to check out git repo
export repo_branch="worker"

export root_name="frog-worker" #name you give to your project in ecs env
export rootdns=10academy.org

export dnsprefix="frog-worker"  #used to create dns name
if [ "$ENV" == "dev" ]; then
...
export ecsTaskPortMapList=5000  #all ports to expose
