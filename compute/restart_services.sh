...
if ${mounts3:-true}; then
    ...
    (crontab -l 2>/dev/null; echo "@reboot  BUCKET=$BUCKET /bin/bash ${maindir}/s3mount/mount-s3fs.sh") | crontab -
fi

#add jupyterhub restart if required
if ${setupjhub:-false}; then
    echo "       > restart jupyterhub service .."
    (crontab -l 2>/dev/null; echo "@reboot  systemctl restart jupyterhub") | crontab -
fi
