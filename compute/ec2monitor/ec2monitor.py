...
print('Welcome to Tenx EC2 Monitor!')
        ...
        if 'Reservations' in response and len(response['Reservations']) > 0:
            ...
            for instance in response['Reservations'][0]['Instances']:
                ...
                if 'InstanceStatuses' in status_response and len(status_response['InstanceStatuses']) > 0:
                    res.update(flatten_dict({'Status_Check':status_response['InstanceStatuses'][0]}))
                    #res.update(res2)
                else:
                    res['Status_Check'] = "Unknown"  # No status check details found          
                    
                print(f"Instance details: {res}")      
...
def check_and_start_ec2_instance(instance_ids=[], name_tags=""):
    ...
    if get_day_of_week('Sunday'):  #get_day_of_week('Saturday') or 
        print(f"Today is {get_day_of_week()} so we are not starting any EC2 instance")
        return {'status': 'not_started', 'message': f"Today is {get_day_of_week()} so we are not starting any EC2 instance"}
    ...
    try:
        for tag, instance_id in instance_ids.items():
            ...
            if 'group' in tag:
                iidinfo['status'] = 'unableToStart'
                iidInfo['message'] = f"You can not start an EC2 instance with ID: {instance_id} that is currently in state={instance_state}"
                
                print(f"Instances with tag='group' is not allowed to start")
                ...
                ip_address = []
                iloop = 0
                print(f'Waiting for public IP address ...')
                while len(ip_address)==0:    
                    print(f'time (sec) = {iloop}')     
                    response = ec2_client.describe_instances(InstanceIds=[instance_id])           
...
if '__main__' == __name__:
    ...
    while True:
        ...
        for name in name_tags:
            print(f'==========Checking Idle State for time={datetime.now()}, Tag={name}===============')
            print()
        ...
        time.sleep(240)  # Check every minute
